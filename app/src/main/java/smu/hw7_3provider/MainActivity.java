    ...
    private Runnable getGalleryData= new Runnable() {
        public void run() {
            try {
                //원하는 자료 처리(데이터 로딩등)
                try {
                    springDataList = readImageInMyGallery_final(1); // 이건 커서 맨 앞에서 시작
                    summerDataList = readImageInMyGallery_final(2); // 이건 커서 맨 앞에서 시작
                    fallDataList = readImageInMyGallery_final(3); // 이건 커서 맨 앞에서 시작
                    winterDataList = readImageInMyGallery_final(4); // 이건 커서 맨 앞에서 시작
...
/////////////////////
    ...
    private ArrayList<Moment> readImageInMyGallery_final(int whatSeason) throws ParseException {
        ...
        switch (whatSeason){
            case 1:
                // 봄일 때
               // title_month[0] ="202203";
                //title_month[1]= "202204";
               // title_month[2]="202205";
